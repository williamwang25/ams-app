/**
 * borrow.reject：管理员审批拒绝。
 *
 * 契约：docs/04-api-spec.md 4.2.3.3
 * 流程：docs/07-workflows.md 7.5.3
 *
 * 事务化：状态校验 → 更新申请 → 批量恢复资产 PENDING→IDLE，current_borrow_id=null → 写日志。
 *
...
module.exports = async (event) => {
  ...
  const reject_reason = String(data.reject_reason || '').trim();
  if (!borrow_id) return err(1001, '缺少 borrow_id');
  if (!reject_reason) return err(1001, '请填写拒绝原因');
  if (reject_reason.length > REASON_MAX) {
    return err(1001, `拒绝原因不超过 ${REASON_MAX} 字`);
  }
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (borrow.status !== 'PENDING') {
        const e = new Error(`当前状态 ${borrow.status} 不允许拒绝`);
        e.bizCode = 3002;
        throw e;
      }
    ...
    console.error('[borrow.reject] transaction failed:', e);
    return err(5001, '操作失败，请稍后重试');
  }

  return ok({ _id: borrow_id, status: 'REJECTED' });
};
